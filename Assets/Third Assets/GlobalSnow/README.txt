...
Current version
- Added "Big World Antiflicker". Enabled by default, this option now let you disable the camera relative rendering

v7.0
- Added snow profiles support. Create/copy/reuse settings stored in scriptable objects called "Profiles" (create them from Global Snow inspector)

v6.9.1
- API: added "GetSnowAmountAt(worldPosition)" which returns a value > 0 if there's snow at a given position or 0 if it's clear
